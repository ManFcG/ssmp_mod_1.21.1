...
/**
 * Obsługuje slash command {@code /playtime} na Discordzie.
 *
 * Skrócona wersja {@code /stats} pokazująca wyłącznie czas gry.
 */
public class PlaytimeCommand {
    ...
    /**
     * Główna metoda obsługi komendy {@code /playtime}.
     *
     * @param event  zdarzenie slash command (deferReply powinno być wywołane wcześniej)
     * @param bridge instancja DiscordBridge
     */
    public static void handle(SlashCommandInteractionEvent event, DiscordBridge bridge) {
        ...
        try {
            ...
            LOGGER.warn("[SSMP] Błąd odczytu playtime: {}", e.getMessage());
            event.getHook().sendMessageEmbeds(new EmbedBuilder()
    ...
    private static void handleByDiscordUser(SlashCommandInteractionEvent event,
        ...
        sendPlaytimeEmbed(event, al.mcUsername(), al.mcUuid(), statsReader, server);
    }
    ...
    private static void handleByMcNick(SlashCommandInteractionEvent event,
        ...
        if (cacheReader != null) {
            ...
            if (uuid.isPresent()) {
                sendPlaytimeEmbed(event, nick, uuid.get(), statsReader, server);
                return;
            }
        }
    ...
    private static void sendPlaytimeEmbed(SlashCommandInteractionEvent event,
                                           String mcUsername,
                                           String mcUuid,
                                           PlayerStatsReader statsReader,
                                           MinecraftServer server) throws IOException {
        ...
        MessageEmbed embed = StatsEmbedBuilder.buildPlaytime(mcUsername, mcUuid, statsOpt.get(), online);
